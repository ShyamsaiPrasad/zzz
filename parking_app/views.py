...
def vehicles_entry(request):
    ...
    page_num = request.GET.get('pagee')  # creating the total pages
    paginator = Paginator(vehi, 5)  # setting total number of products in a page : 2
    try:
...
def search(request):
    ...
    if search_query == None:
        ...
        vehicles = vehicle_entry.objects.filter(vehicle_no__icontains=search_query)
    context = {'vehicles': vehicles}
...
def logouts(request):
    ...
    if user_id is not None:
        # You can perform logging operations here if needed
        pass
    logout(request)
    return redirect('login')
